...
module mod_minimize
  ...
  public :: minimize_spec, minimize, minimize_norm, minimize_norm_single
...
contains
  ...
  subroutine minimize_spec(n, m, x, lb, ub, line, dim_v, n_gauss, maxiter, iprint)
    ...
    do while(task(1:2).eq.'FG'.or. task.eq.'NEW_X' .or. task.eq.'START') 
       ...
       if (task(1:2) .eq. 'FG') then          
          ...
          print*, "minimize_spec not available"
          stop
  ...
  subroutine minimize(n, m, x, lb, ub, cube, n_gauss, dim_v, dim_y, dim_x, lambda_amp, lambda_mu, lambda_sig, &
    ...
    do while(task(1:2).eq.'FG'.or. task.eq.'NEW_X' .or. task.eq.'START') 
       ...
       if (task(1:2) .eq. 'FG') then          
          ...
          print*, "f_g_cube_fast not avilable"
          stop
       elseif (task(1:5) .eq. 'NEW_X') then
          !        1) Terminate if the total number of f and g evaluations
          !             exceeds maxiter.
  ...
  ! Minimize algorithn for a cube with regularization
  subroutine minimize_norm(n, m, x, lb, ub, cube, n_gauss, dim_v, dim_y, dim_x, lambda_amp, lambda_mu, lambda_sig, &
       lambda_var_amp, lambda_var_mu, lambda_var_sig, lambda_lym_sig, maxiter, kernel, iprint, std_map, lym, c_lym, &
    ...
    do while(task(1:2).eq.'FG'.or. task.eq.'NEW_X' .or. task.eq.'START') 
       ...
       if (task(1:2) .eq. 'FG') then          
          !     Compute function f and gradient g for the sample problem.
          call f_g_cube_fast_norm(f, g, cube, x, dim_v, dim_y, dim_x, n_gauss, kernel, lambda_amp, lambda_mu, lambda_sig, &
    ...
    deallocate(iwa)
    deallocate(wa)
  end subroutine minimize_norm

  ! Minimize algorithn for a cube with regularization
  subroutine minimize_norm_single(n, m, x, lb, ub, cube, n_gauss, dim_v, dim_y, dim_x, lambda_amp, lambda_mu, lambda_sig, &
       lambda_var_amp, lambda_var_mu, lambda_var_sig, lambda_lym_sig, maxiter, kernel, iprint, std_map, lym, c_lym, &
       lambda_sig_corr_narrow, lambda_sig_corr_broad, lambda_mu_corr_narrow, lambda_mu_corr_broad, lambda_r, params_lsf, &
       delta_1, delta_2, delta_3, delta_4)
    implicit none      
    ...
    do while(task(1:2).eq.'FG'.or. task.eq.'NEW_X' .or. task.eq.'START') 
       ...
       if (task(1:2) .eq. 'FG') then          
          !     Compute function f and gradient g for the sample problem.
          call f_g_cube_fast_norm_single(f, g, cube, x, dim_v, dim_y, dim_x, n_gauss, kernel, lambda_amp, lambda_mu, lambda_sig, &
               lambda_var_amp, lambda_var_mu, lambda_var_sig, std_map, lambda_sig_corr_narrow, lambda_sig_corr_broad, &
               lambda_mu_corr_narrow, lambda_mu_corr_broad, lambda_r, params_lsf, delta_1, delta_2, delta_3, delta_4)
       elseif (task(1:5) .eq. 'NEW_X') then
          !        1) Terminate if the total number of f and g evaluations
    ...
    deallocate(iwa)
    deallocate(wa)
  end subroutine minimize_norm_single
