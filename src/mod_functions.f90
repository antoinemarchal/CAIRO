...
contains
  ...
  subroutine init_bounds(line, n_gauss, dim_v, lb, ub, lb_sig, ub_sig, ib_sig, lb_amp, ub_amp)
  ...
  subroutine update(cube, params, b_params, n_gauss, dim_v, dim_y, dim_x, lambda_amp, lambda_mu, lambda_sig, &
       ...
       iprint, std_map, lym, c_lym, norm_var, lambda_sig_corr_narrow, lambda_sig_corr_broad, &
    ...
    real(xp), intent(in) :: lambda_sig_corr_narrow
    real(xp), intent(in) :: lambda_sig_corr_broad
    real(xp), intent(in) :: lambda_mu_corr_narrow
    real(xp), intent(in) :: lambda_mu_corr_broad

    real(xp), intent(in) :: lambda_r

    real(xp), intent(in) :: lb_sig !! lower bound sigma
    real(xp), intent(in) :: ub_sig !! upper bound sigma
    real(xp), intent(in) :: ib_sig !! intermediate bound sigma
    real(xp), intent(in) :: lb_amp !! lower bound amp
    real(xp), intent(in) :: ub_amp !! upper bound amp
    ...
    if (norm_var .eqv. .false.) then
       print*, "norm var == .false. nor available"
       stop
