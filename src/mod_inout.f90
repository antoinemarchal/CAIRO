...
contains
  ...
  subroutine read_parameters(filename_parameters, filename, fileout, timeout, filename_noise, filename_init_spec, n_gauss, &
       ...
       regul, descent, lstd, ustd, iprint, iprint_init, save_grid, lym, init_spec, norm_var, lambda_sig_corr_narrow, &
       lambda_sig_corr_broad, lambda_mu_corr_narrow, lambda_mu_corr_broad, ib_sig, lambda_r, lb_amp, ub_amp, filename_lsf)
    implicit none
    ...
    real(xp), intent(inout) :: lambda_sig_corr_narrow, lambda_sig_corr_broad
    real(xp), intent(inout) :: lambda_mu_corr_narrow, lambda_mu_corr_broad
    real(xp), intent(inout) :: lambda_r    
    real(xp), intent(inout) :: amp_fact_init, sig_init
    real(xp), intent(inout) :: ub_sig_init, ub_sig, ib_sig
    real(xp), intent(inout) :: lb_sig_init, lb_sig
    real(xp), intent(inout) :: lb_amp, ub_amp
    logical, intent(inout) :: noise, regul, descent, save_grid
    logical, intent(inout) :: lym, init_spec
    logical, intent(inout) :: norm_var
    ...
    character(len=512), intent(inout) :: filename_lsf
    character(len=8), intent(inout) :: init_option

    namelist /user_parameters/ filename, fileout, timeout, filename_noise, filename_init_spec, n_gauss, n_gauss_add, &
         lambda_amp, lambda_mu, lambda_sig, lambda_var_amp, lambda_var_mu, lambda_var_sig, lambda_lym_sig, amp_fact_init, &
         sig_init, lb_sig_init, ub_sig_init, lb_sig, ub_sig, init_option, maxiter_init, maxiter, m, noise, regul, &
         descent, lstd, ustd, iprint, iprint_init, save_grid, lym, init_spec, norm_var, lambda_sig_corr_narrow, &
         lambda_sig_corr_broad, lambda_mu_corr_narrow, lambda_mu_corr_broad, ib_sig, lambda_r, lb_amp, ub_amp, filename_lsf
